...
def test_named_access(calc):
    ...
    assert (r.uncorrected, r.wigner, r.infinite_parabola) == \
...
def test_to_dict_kie(calc):
    ...
    assert "C5" not in d
    assert set(d["C1"]) == {"uncorrected", "wigner", "infinite_parabola"}
...
def test_to_csv_roundtrip(calc, tmp_path):
    ...
    assert lines[0] == "name,uncorrected,wigner,infinite_parabola"
...
def test_to_dataframe(calc):
    ...
    assert list(df.columns) == ["name", "uncorrected", "wigner", "infinite_parabola"]
